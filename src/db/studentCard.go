...
func GetStudentCards(db *sql.DB) ([]StudentCard, error) {
	...
	rows, err := db.Query("SELECT people.id, people.fio, people.gender, people.img, people.have_access, student.id_group, groups.name, groups.id_direction, direction.id_cathedra, cathedra.id_faculty, faculty.id_institute FROM people, student, groups, direction, cathedra, faculty WHERE people.id = student.id_people AND groups.id = student.id_group AND direction.id = groups.id_direction AND cathedra.id = direction.id_cathedra AND faculty.id = cathedra.id_faculty ORDER BY have_access DESC")
	if err != nil {
	...
	for rows.Next() {
		...
		err := rows.Scan(&card.Id, &card.Fio, &card.Gender, &card.Photo, &card.HaveAccess, &card.IdGroup, &card.GroupName, &card.IdDirection, &card.IdCathedra, &card.IdFaculty, &card.IdInstitute)
		if err != nil {
			return []StudentCard{}, err
		}
