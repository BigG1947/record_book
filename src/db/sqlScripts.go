...
const updateAccessionDataScript = `UPDATE accession SET edit_access = ?, set_absence = ?, get_absence = ?, set_mark = ?, set_event = ?, get_sensitive = ?, set_sensitive = ?, manage_load = ? , manage_academ = ? WHERE id_people = ?;`
...
const getAllLoadsByIdScript = `SELECT loads.id,
       d.id,
       d.name,
       p.id,
       p.fio,
